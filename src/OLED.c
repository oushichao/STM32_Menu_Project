...
void OLED_WriteCommand(uint8_t command){
    OLED_Start();
    OLED_SendByte(0x78);// OLED I2C address
...
/**
  ...
  * @retval 无
  */
 // OLED的显示区域被分为8行，每行128列，每个字节对应8个像素点，
 //所以Y坐标范围是0~7，X坐标范围是0~127
void OLED_SetCursor(uint8_t Y, uint8_t X)
{
	OLED_WriteCommand(0xB0 | Y);					//设置Y坐标
	OLED_WriteCommand(0x10 | ((X & 0xF0) >> 4));	//设置X坐标的高4位
	OLED_WriteCommand(0x00 | (X & 0x0F));			//设置X坐标的低4位
}
/**
  * @brief  OLED清屏
  * @param  无
